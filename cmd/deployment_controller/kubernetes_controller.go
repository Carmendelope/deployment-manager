...
func main() {
    ...
    var err error
    if internal {
        clientset, err = getInternalKubernetesClient()
    } else {
        clientset, err = getExternalKubernetesClient()
    }
    ...
    //podListWatcher := cache.NewListWatchFromClient(clientset.CoreV1().RESTClient(), "services", v1.NamespaceDefault, fields.Everything())
